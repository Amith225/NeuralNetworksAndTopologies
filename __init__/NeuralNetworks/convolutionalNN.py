...
class ConvolutionalNN(AbstractNN):
    def _forwardPass(self, layer=1):
        ...
        if layer < self.shape.LAYERS - 1:
            ...
            self.outputs[layer + 1] = self.ann.process(self.outputs[layer].flatten())
    ...
    def __init__(self,
                 ...
                 annInitializer: "AbstractInitializer",
                 activators: "Activators",
                 annActivators: "Activators",
                 lossFunction: "AbstractLossFunction",
                 annLossFunction: "AbstractLossFunction",
                 strides,
        ...
        self.ann = ArtificialNN(annShape, annInitializer, annActivators, annLossFunction)
        del self.ann.train
        del self.ann.test
    ...
    def __crossCorrelate(array, pad, outShape, stride, kernel, poolType=None):
        ...
        if poolType is None:
            ...
            if poolType == ConvolutionalNN.PoolingType.MAX:
                crossed = strided.max(axis=(3, 4))
            elif poolType == ConvolutionalNN.PoolingType.MEAN:
                crossed = strided.mean(axis=(3, 4))
            else:
                raise ValueError("poolType argument is not valid")
